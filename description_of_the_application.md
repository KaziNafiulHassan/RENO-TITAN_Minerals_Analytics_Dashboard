...
####4. Material Flow Analysis (Sankey Diagrams)This is the most advanced analytical feature, likely used for "industrial ecology" studies. It uses **Sankey diagrams** to model how raw ores are converted into final products.
...
###Special Focus: Vietnam Data HubThe document includes a specific module for **Vietnam**, which serves as a case study or "template" that could later be applied to other countries like Bangladesh. This involves compiling specific local mining data and reconciling it with the global trade models.

###Summary of User ExperienceThe user (a scientist or planner) would log in to:

1. Check **Production** trends to see who is mining the most Zircon or Titanium.
2. Switch to **Trade** view to see where that ore is going and if the trade data is reliable (Mirror check).
3. Use the **Material Flow** tool to estimate how much of that ore is actually being processed into metal or chemicals within a specific country versus being exported raw.

---

**Would you like me to help you define the specific data fields required for the "Mass Balance" equation (Section 4), or perhaps draft a schema for the database based on the sources listed?**